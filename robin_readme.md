!This is not the original readme but an attempt at explaining what is going on by me (Robin, Student Research Assistant at UTN's Data Systems Lab)

# Explanation of the Pollock Benchmark Structure

## 0. Benchmark Overview
1. The polluter writes polluted versions of the ```results/source.csv``` file into ```polluted_files/csv/```. It also writes the expected output of files that are read with the correct grammar (which is known by the polluter) into ```polluted_files/clean/```. These serve as the basis for comparison with what the SuTs have read from the polluted files later. On top of this, the polluter also writes the dialect information (e.g. delimiter, column datatypes, quote character etc.) into ```polluted_files/parameters/```
2. The different SuTs read the files from ```polluted_files/csv/```. 
...
4. The evaluation script ```evaluate.py``` uses (kind of expensive) Multi-Set operations to compare the outputs of the SuTs (```results/<sut>/polluted_files/loading/```) with the expected clean outputs in (```polluted_files/clean/```). It does so on a per-row (record) and per-cell basis. The final score is a mix of loading-success and recall + precision metrics (for formula, see the more detailed explanation of the Evaluation below)
...
Additionally, it does things like adding additional stray quote characters into fields or leave out a separator. These pollutions can change what the semantic content of a file is, which is why the benchmark has to save a clean version of each polluted file in ```polluted_files/clean/```.
...
Every SuT tries to read the polluted files in ```polluted_files/csv/```. After it is read into the SuT, it is dumped to ```results/<sut>/polluted_files/loading/``` using a shared csv-dialect (the one by pandas .to_csv() function).

**Some of the systems (e.g. duckdbparse) are given the dialect** info from ```polluted_files/parameters/```, others (e.g. duckdbauto or clevercsv) infer them automatically. In general, the benchmark tried to be a "best effort" benchmark, meaning that the benchmark score directly correlates with the number of settings a given SuT has to deal with different dialects. This is why comparisons between SuTs only make sense if they are either both using the supplied metadata (e.g. duckdbparse, sqlite) or not using it at all (e.g. duckdbauto, clevercsv).
...
```
Each component can is from [0,1], so the maximum score is 10.
